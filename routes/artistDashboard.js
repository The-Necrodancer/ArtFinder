...
import { addPhotosToPortfolio, getArtistById, updateArtistProfile } from "../data/artists.js";
const router = Router(); 
...
router.get("/", roleMiddleware(["artist"]), async (req, res) => {
    try {
...
.post('/', async (req, res) => {
    ...
    try {
        if(updatingPortfolio && adding ) {
            ...
            if(index === -1) throw "Error: image being removed does not exist in portfolio"; 
