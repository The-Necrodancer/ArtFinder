...
export const updateArtistProfile = async (aid, newProfile) => {
  ...
  if (!Object.keys(newProfile).length)
    throw "Error: newProfile cannot be empty.";
  ...
  if ("portfolio" in newProfile) {
    if (!Array.isArray(newProfile.portfolio))
      throwWrongTypeError("portfolio", "Array", typeof newProfile.portfolio);
  ...
  if ("pricingInfo" in newProfile) {
    ...
    if (
      !newProfile.pricingInfo ||
      newProfile.pricingInfo.constructor !== Object
    ) {
      throwWrongTypeError(
        "pricing info",
        "Object",
        typeof newProfile.pricingInfo
      );
    }
  ...
  if ("availability" in newProfile) {
    if (typeof newProfile.availability !== "boolean") {
      throwWrongTypeError(
        "availability",
        "boolean",
        typeof newProfile.availability
      );
    }
